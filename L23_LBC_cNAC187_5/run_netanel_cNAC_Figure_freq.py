...
def run_step(step_number, plot_traces=None):
    ...
    ax2.set_xlim([1000,neuron.h.tstop]) # 1000 # 50
    ...
    ax1.text(-50,271,'c', fontsize=16, fontproperties=font)
    #ax1.text(850,271,'D', fontsize=15)
    fig.tight_layout()
    fig.subplots_adjust(wspace=0.06, bottom=0.09)
    fig.savefig('Figure_freq400.pdf') # 300
    #fig.savefig('/home/userlab/neuron/figures/cNAC_temp_200.pdf') 
...
def main(plot_traces=True):
    ...
    #Ent = []
    #Enct = []
    for step_number in range(1): # 12,13): #range(1, 2):
        #run_step(step_number, plot_traces=plot_traces)
        #fig, ((ax1 ,ax2), (ax3 ,ax4)) = plt.subplots(2, 2, sharey=True, figsize=(6.4,4.8)) # , figsize=(8,6)) # width , high    6.4,4.8 
        run_step(step_number, plot_traces=plot_traces)
        #ax1 = ff
        #fig.savefig('/home/userlab/BBP/NeuroMorpho/recons/Figure_freq.pdf') 
        #fig.show()
