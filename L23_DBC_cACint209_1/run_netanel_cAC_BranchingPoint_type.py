...
def create_cell(add_synapses=True):  # """Create the cell model"""
    neuron.h.load_file("morphology.hoc") # Load morphology
    neuron.h.load_file("biophysics.hoc") # Load biophysics
    neuron.h.load_file("template.hoc") # Load main cell template
    #print("Loading cell cACint209_L23_DBC_4114c4c36c") # Instantiate the cell from the template
...
def run_step(step_number, plot_traces=None):
    ...
    for sec in range(len(tbl)-1):
      ...
      #print nn
      xy.append(nn)
      #xy.append(str(tbl[sec+1][2]).replace("cACint209_L23_DBC_4114c4c36c[0].axon[", "").replace("]",""))
	...
	if (cc[iii+1] > cc[iii]+40): # discard close peaks
      ...
      #aaa.append(np.array(j3) - j3[0] + 50/neuron.h.dt) # alignment
    ...
    BP_same = 0 # branching point when the response identical in the two daughter branches
    BP_diff = -1 # branching point when the response different in the two daughter branches
    kk=-1
    ...
    ax2.set_xlim([1000,neuron.h.tstop])
    ...
    fig.savefig('cAC-neuron-freq2.pdf')
    #fig.show()
    #return([En,Enc])
...
def main(plot_traces=True):
    ...
    for step_number in range(1):
        ...
        #Ent.append(En[0])
        #Enct.append(En[1])
        #Ent.append(run_step(step_number, plot_traces=plot_traces))
