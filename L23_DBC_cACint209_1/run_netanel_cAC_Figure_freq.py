...
def raster(event_times_list, cl , **kwargs):
    ...
    plt.ylim(-7.5, len(event_times_list) - .5)
    return ax
...
def run_step(step_number, plot_traces=None):
    ...
    cll = ['maroon' ,'green', 'orange' , 'deepskyblue' , 'violet' , 'navy' , 'dimgray' , 'silver'] # green instead of yellow
    ...
    ax1.text(-50,271,'d', fontsize=15, fontproperties=font)
    #
    fig.tight_layout()
    fig.subplots_adjust(wspace=0.06, bottom=0.09)
    fig.savefig('Figure_cAC_freq400.pdf')
    #fig.show()
    #return([En,Enc])
