...
def create_stimuli(cell, step_number):
    ...
    freq = 120 # 2.8 #10 #3 #2.8 #8 # 125 Hz
    interval = round(((1000.0/freq)/dtt))*dtt
    pulses = []
    st = {}
    for i in range(1000): # 1 for step
...
def run_step(step_number, plot_traces=None):
    ...
    for sec in range(len(tbl)-1):
      ...
      xy.append(str(tbl[sec+1][2]).replace("bNAC219_L23_LBC_fe2122c75c[0].axon[", "").replace("]",""))
    ...
    ax4.plot(recordings['time'], recordings['v_vect142'] , color='violet',lw=0.1) # 142 instead of 161
    ax4.set_ylabel(' ')
    ...
    for i in range(len(ll)):
      if (int(ll[i][1])>1):
        ax7.plot([ll[i][3], ll[i][8]],[-(float(ll[i][2])), -float(ll[i][7])], c=clr[int(ll[i][1])]  , lw = float(ll[i][5])*6)  # For Cauli Basket
